...
export default function Editor() {
  ...
  const [pages, setPages] = useState([]); // [{id, page_number}]
  const [saveState, setSaveState] = useState('idle'); // idle | saving | saved
  const counterRef = useRef(0);
  const loadedRef = useRef(false); // guards against autosaving while the initial load is still running

  // Load auth + ownership state, then load-or-create the user's scrapbook page
  // and restore whatever was placed on it last time.
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      // Find the user's first scrapbook, or create one on first visit.
      ...
      // Load all pages, or create page 1 if this scrapbook has none yet.
      ...
      // Restore whatever was placed on that first page.
      ...
      if (placed?.length) {
        const restored = placed.map((row) => {
          ...
          return {
            uid: row.id,
            itemId: row.element_id,
            packId: findItemPack(row.element_id),
            x: row.x, y: row.y, w: row.w, h: row.h, z: row.layer_order,
            isFont: CATALOG.fonts.some((p) => p.items.some((it) => it.id === row.element_id)),
          };
        });
      ...
      // If we just came back from a successful checkout, the webhook may take
      // a moment to land — refetch ownership/credits shortly after landing.
  ...
  // Autosave: whenever the placed elements change (after the initial load),
  // replace the page's saved rows with the current layout.
  useEffect(() => {
  ...
  async function switchPage(pid) {
    ...
    const supabase = getSupabaseBrowserClient();
    if (!supabase) return;
    loadedRef.current = false; // pause autosave while we swap the loaded elements out from under it
    setPageId(pid);
    setElements([]);
